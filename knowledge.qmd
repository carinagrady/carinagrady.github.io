...
# load libraries and function here
...
# function to create timeline plots with interactive tooltips
create_historical_plotly <- function(data, 
                                 y_var, x_start, x_end, fill_var, 
                                 tooltip_cols, title, y_label, fill_label) {
  
  # Wrap text for multiple tooltip columns
  data <- data %>%
    mutate(region = case_match(
      region,
      "Middle East & North Africa" ~ "Middle East\n& North Africa",
      "Sub-Saharan Africa" ~ "Sub-Saharan\nAfrica",
      .default = region  # Keeps other values unchanged
    )) %>%
    rowwise() %>%  # Ensure text is created row-wise
    mutate(tooltip_text = paste(
      "<b>", y_label, ":</b>", !!sym(y_var), "<br>",
      "<b>Years:</b>", !!sym(x_start), "to", !!sym(x_end), "<br>",
      paste0("<b>", tooltip_cols, ":</b> ", c_across(all_of(tooltip_cols)), collapse = "<br>")
    )) %>%
    ungroup()
  ...
  p <- ggplot(
    data = data,
    aes(
      y = reorder(.data[[y_var]], .data[[x_start]]),
      xmin = .data[[x_start]], xmax = .data[[x_end]],
      fill = .data[[fill_var]],
      text = tooltip_text  # Uses pre-computed column
    )) +
    geom_rect(
      aes(
        xmin = .data[[x_start]], xmax = .data[[x_end]],
        ymin = as.numeric(reorder(.data[[y_var]], .data[[x_start]])) - 0.4, 
        ymax = as.numeric(reorder(.data[[y_var]], .data[[x_start]])) + 0.4
      ),
      color = "black", alpha = 0.9
    ) +
    scale_fill_manual(values = colors) +
    scale_x_continuous(
      breaks = seq(-3000, 2000, 500),  # Major tick marks every 500 years
      minor_breaks = seq(-3000, 2000, 100),  # Minor tick marks every 100 years
      labels = function(x) ifelse(x < 0, paste(abs(x), "BCE"), paste(x, "CE"))
    ) +
    labs(
      title = title,
      x = "Year",
    ...
    theme(
      ...
      axis.text.x = element_text(angle = 45, hjust = 1),  # Tilts x-axis labels for readability
      axis.ticks.x = element_line(color = "black")  # Ensures tick marks are visible
    )
...
```{r, echo=FALSE, message=FALSE}
historical_ages <- read_csv("data/historical_ages.csv", show_col_types = FALSE)

era_events <- read_csv("data/historical_era_events.csv", show_col_types = FALSE)

civilizations <- read_csv("data/historical_civilizations.csv", show_col_types = FALSE) %>% 
  mutate(Contributions = wrap_text(Contributions, 40))

religions <- read_csv("data/historical_religions.csv", show_col_types = FALSE) %>% 
  mutate(
    Beliefs = wrap_text(Beliefs, 40),
    `Key Civilizations` = wrap_text(`Key Civilizations`, 40)
  )

colors <- c(
  "Americas" = "#8C564B",  # Muted brown
  "Asia" = "#2E8B57",  # Muted dark green
  "Europe" = "#4682B4",  # Muted steel blue
  "Middle East\n& North Africa" = "#D2691E",  # Muted dark orange
  "Sub-Saharan\nAfrica" = "#6A5ACD"  # Muted slate blue
)

```

stone -\> ancient -\> medieval -\> modern

generally....
pre-history
ancient Greece & the Roman Empire
feudalism, religion (dark ages)
Enlightenment
Globalization
Science

below is more info on what exactly happened and over what time period

```{r, echo=FALSE}
era_events %>%
  mutate(`Number of Years` = comma(`Number of Years`)) %>% 
  rename(`# of Years` = `Number of Years`) %>% 
  kable(format = "html") %>%
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))

```

this is all fine and dady, but what were the civilizations? they had names we will only focus on history (not prehistory) because we don't know much about what happened in prehistory since nothing was recorded

the rise & fall of civilizations across the eras

```{r, echo=FALSE}
create_historical_plotly(
  data = civilizations,
  y_var = "key_civilizations",
  x_start = "start_year",
  x_end = "end_year",
  fill_var = "region",
  tooltip_cols = c("Empires/States", "Rise and Fall Years", "Contributions"),
  title = "Timeline of Major Civilizations by Region",
  y_label = "Civilizations",
  fill_label = "Regions"
)
```

and here are their religions
...
create_historical_plotly(
  ...
  x_end = "end_year",
  fill_var = "region",
  tooltip_cols = c("Rise and Fall Years", "Key Civilizations", "Beliefs"),
  title = "Timeline of Major Religions by Region",
  y_label = "Religions",
  fill_label = "Regions of Origin"
)
...
-   how history often repeats itself every 250 years (Strauss-Howe generational theory)
